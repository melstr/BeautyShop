...
{
    public partial class changeAppointmentMaster : Form
    {
        ...
        private void LoadData2(bool today = false)
        {
            try
            {
                ...
                if (!today)
                {
                    sqlDataAdapter2 = new SqlDataAdapter("SELECT *, 'Завершить' AS [Command] FROM finished_appointments WHERE master_id = @ms", GlobalConnection.connection);
                    sqlDataAdapter2.SelectCommand.Parameters.Add("@ms", SqlDbType.Int).Value = GlobalConnection.AuthedUser.Id;
                    sqlBuilder2 = new SqlCommandBuilder(sqlDataAdapter2);

                }
                else
                {
                    sqlDataAdapter2 = new SqlDataAdapter("SELECT *, 'Завершить' AS [Command] FROM finished_appointments WHERE day_date = @dt AND master_id = @ms", GlobalConnection.connection);
        ...
        private void LoadData1(bool today = false)
        {
            try
            {
                if (!today)
                {
                    sqlDataAdapter1 = new SqlDataAdapter("SELECT *, 'Завершить' AS [Command] FROM appointments WHERE master_id = @ms", GlobalConnection.connection);
                    sqlDataAdapter1.SelectCommand.Parameters.Add("@ms", SqlDbType.Int).Value = GlobalConnection.AuthedUser.Id;
                    sqlBuilder1 = new SqlCommandBuilder(sqlDataAdapter1);
                }
                else
                {
                    sqlDataAdapter1 = new SqlDataAdapter("SELECT *, 'Завершить' AS [Command] FROM appointments WHERE day_date = @dt AND master_id = @ms", GlobalConnection.connection);
        ...
        public changeAppointmentMaster(Form pv)
        {
            PrevForm = pv;
            InitializeComponent();
        }

        private void changeAppointmentMaster_Load(object sender, EventArgs e)
        {
            LoadData1();
            LoadData2();
        }
        ...
        private void changeAppointmentMaster_FormClosed(object sender, FormClosedEventArgs e)
        {
            PrevForm.Show();
        }
    }
